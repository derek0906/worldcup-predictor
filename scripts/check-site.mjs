...
const rewardsFunctionSource = await readFile("netlify/functions/rewards.mjs", "utf8");
...
assert.match(indexSource, /id="rewardBanner"/, "daily leaderboard should show the 100 RMB reward banner");
assert.match(indexSource, /id="rewardClaimForm"/, "daily winner should have a reward claim form");
...
assert.match(appSource, /DAILY_REWARD_AMOUNT/, "app.js should define the daily reward amount");
assert.match(appSource, /renderRewardBanner/, "app.js should render daily reward state");
assert.match(appSource, /submitRewardClaim/, "app.js should submit reward claims");
...
assert.match(stylesSource, /@media \(max-width: 520px\)[\s\S]*\.site-header\s*\{[\s\S]*flex-direction:\s*column/, "mobile header should stack so the status pill does not squeeze the title");
assert.match(stylesSource, /@media \(max-width: 520px\)[\s\S]*\.page-shell\s*\{[\s\S]*100% - 12px/, "mobile page shell should use tighter side gutters");
...
assert.match(leaderboardStoreSource, /dailyRewards/, "leaderboard API should return daily reward status");
assert.match(leaderboardStoreSource, /saveRewardClaim/, "leaderboard storage should persist reward claims");
assert.match(leaderboardStoreSource, /sanitizeRewardClaim/, "leaderboard storage should sanitize reward claims");
...
assert.match(rewardsFunctionSource, /saveRewardClaim/, "rewards function should save winner claim details");
assert.match(rewardsFunctionSource, /claimCode/, "rewards function should return a claim code for manual payout");
